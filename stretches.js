...
function key(){const area=document.getElementById('workoutArea');if(!area)return'';return [document.querySelector('#workoutArea h2')?.textContent||'',document.querySelector('#weekBanner .weekTitle')?.textContent||'',...Array.from(area.querySelectorAll('.exercise h3')).map(h=>h.textContent)].join('|')}
...
function init(){styles();render();const area=document.getElementById('workoutArea');if(area){let queued=false;new MutationObserver(()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;render()})}).observe(area,{childList:true,subtree:true})}}
